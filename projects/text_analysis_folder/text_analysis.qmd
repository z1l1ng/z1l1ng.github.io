...
This project uses R to create explore titles, genres, and ratings of movies and tv shows of different streaming services. 

To view the qmd, click [here](PUT QMD INSIDE).

# Introduction
Do you ever wonder specifically what types of movies or tv shows are on the streaming services you pay for? We will be exploring a comprehensive collection of all titles for Amazon Prime, Apple TV, HBO, Hulu, and Netflix using data collected from IMDB and published on Kaggle. Merging together these datasets we end up with 120,930 rows and 9 columns. Each row being title, and some columns representing the type, being movie or tv show, the year it was released, genre of the title, etc. 
...
We can see that for most titles to least, it ranks Amazon Prime, Netflix, Apple TV, Hulu and HBO. From our 120,930 titles, we can see we have more movies than tv shows. 
...
The average length of the title increases over time from average length of 15 in 1925 to average length of 22ish in 2024. We can see that our line of best fit is nearly identical since the length is depends on the words. The average title is around 4.4 words in 2024. This means that the average word is around 5 characters since the average length of our title is 20 in 2024 and the average number of words is 4. 

Exploring the inside our titles, these are the top ten most common words in our titles. 
...
joy_sad <- nrc_sentiments |>
  # filter for joy or sadness
...
For our dataset, we are interested in our genres and how the distribution of words within the titles of each genre reflects the underlying themes and characteristics of different types of movies and TV shows. After filtering for stop words we wil create a dtm for our words and genres, fit our lda model and then interpret our findings. 
...
In conclusion, this analysis provides an exploration of the genres and characteristics of movie and TV show titles across major streaming services such as Amazon Prime, Apple TV, HBO, Hulu, and Netflix. By examining over 120,000 titles, we identified patterns in genre distribution, title length trends over time, and sentiment variations across genres.
